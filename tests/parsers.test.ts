...
describe("parseDuration", () => {
  ...
  it("accepts minutes with м/мин suffix", () => {
    expect(parseDuration("30м")).toEqual({ ok: true, minutes: 30 });
    expect(parseDuration("45 мин")).toEqual({ ok: true, minutes: 45 });
  });

  it("accepts hours with ч suffix", () => {
    expect(parseDuration("1ч")).toEqual({ ok: true, minutes: 60 });
    expect(parseDuration("2 ч")).toEqual({ ok: true, minutes: 120 });
  });

  it("accepts combined hours and minutes", () => {
    expect(parseDuration("1ч30м")).toEqual({ ok: true, minutes: 90 });
  });
